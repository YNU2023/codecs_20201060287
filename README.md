...
源文件的代码组织方式较为杂乱随性，此仓库将杂乱无章的代码有条理的整理出来，通过删改注释，删除重复代码段，并将文件使用面向对象的方式进行重构，将特定的功能封装成类，增加代码的可读性和易用性。使得代码更加清晰易懂。
...
## 三、后记
在修改代码过程中，我的主要工作是将杂乱无章的代码有条理的整理出来，通过删改注释，删除重复代码段，将特定的方法封装成类等，增加代码的可读性和易用性。

**警告：不可避免的是，由于不了解该科研任务的具体要求，不清晰相应模块的具体调用方式等因素，虽然本着不修改具体代码内容，仅改动项目结构的思想，但仍不可避免的改动了少量代码，可能对实验产生影响，导致代码报错。**
